...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 初始化显示当前站点代码（移除重复声明）
    let currentCodeElement = document.getElementById('currentCode'); // 使用 let，避免重复 const
...
async function updateSiteStatusWithTest(source) {
    ...
    if (source === 'custom') {
        ...
        const results = await Promise.all(urls.map(url => testCustomApiUrl(url)));
        const availableCount = results.filter(r => r).length;
        if (availableCount > 0) {
            updateSiteStatus(true);
            if (siteStatus) siteStatus.innerHTML = `<span class="text-green-500">●</span> ${availableCount}/${urls.length} 可用`;
        } else {
            updateSiteStatus(false);
            if (siteStatus) siteStatus.innerHTML = '<span class="text-red-500">●</span> 全部不可用';
        }
        return;
    }
    ...
    try {
        ...
        updateSiteStatus(isAvailable);
        localStorage.setItem(cacheKey, JSON.stringify({ isAvailable, timestamp: Date.now() }));
    } catch (error) {
...
async function testCustomApiUrl(url) {
    if (!url || (CUSTOM_API_CONFIG.validateUrl && !/^https?:\/\/.+/.test(url))) return false;
    ...
    if (CUSTOM_API_CONFIG.cacheResults) {
        ...
        if (cachedResult) {
            try {
                const { isAvailable, timestamp } = JSON.parse(cachedResult);
                if (Date.now() - timestamp < CUSTOM_API_CONFIG.cacheExpiry) return isAvailable;
            } catch (e) {
    ...
    try {
        ...
        const response = await fetch('/api/search?wd=test&customApi=' + encodeURIComponent(url), { signal: controller.signal });
        clearTimeout(timeoutId);
        const result = response.ok;
        if (CUSTOM_API_CONFIG.cacheResults) {
            localStorage.setItem(cacheKey, JSON.stringify({ isAvailable: result, timestamp: Date.now() }));
        }
        return result;
    } catch (e) {
...
function setupEventListeners() {
    ...
    if (apiSource) {
        apiSource.addEventListener('change', async function(e) {
            ...
            if (currentCodeElement) currentCodeElement.textContent = currentApiSource;
            resetSearchArea();
        });
    }
...
window.search = async function() {
    ...
    try {
        ...
        if (currentApiSource === 'custom') {
            ...
            apiParams = customApiUrl.includes(CUSTOM_API_CONFIG.separator) ?
                '&customApi=' + encodeURIComponent(customApiUrl) + '&source=custom&multipleApis=true' :
                '&customApi=' + encodeURIComponent(customApiUrl) + '&source=custom';
        } else {
        ...
        const response = await fetch('/api/search?wd=' + encodeURIComponent(query) + apiParams, { signal: controller.signal });
        clearTimeout(timeoutId);
                    ...
                    <div class="md:flex">
                        ...
                        <div class="p-3 flex flex-col flex-grow ${hasCover ? 'md:w-3/4' : 'w-full'}">
                            <div class="flex-grow">
                                ...
                                <p class="text-gray-400 text-xs line-clamp-2">${(item.vod_remarks || '暂无介绍').toString().replace(/</g, '<')}</p>
                            </div>
